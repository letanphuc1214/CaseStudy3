...
@WebServlet(name = "ServletDeleteProvince", urlPatterns = "/provinces/delete")
public class ServletDeleteProvince extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        request.setCharacterEncoding("utf-8");
        int id = Integer.parseInt(request.getParameter("id"));
        Province existingUser = null;
        try {
            existingUser = provinceDAO.findById(id);
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
        RequestDispatcher dispatcher = request.getRequestDispatcher("/view/admin/dist/delete.jsp");
        request.setAttribute("province", existingUser);
        dispatcher.forward(request, response);
    }
}
